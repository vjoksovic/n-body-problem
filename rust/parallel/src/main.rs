...
fn compute_forces_parallel(bodies: &Vec<Body>, g: f64, eps: f64) -> Vec<[f64; 3]> {
    let n = bodies.len();
...
fn main() {
    ...
    for step in 0..config.steps {
        let forces = compute_forces_parallel(&bodies, config.g, config.eps);
    ...
    println!("Parallel simulation finished.");
}
